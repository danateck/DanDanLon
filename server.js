...
const prompt = `
אתה מסווג מסמכים למערכת Eco-Files.

🗂 יש לך רשימת תיקיות ותתי-תיקיות קבועות.  
מותר לך להשתמש *רק* בשמות הבאים – בדיוק כמו בקוד CATEGORY_KEYWORDS:
...
- "תעודות אחריות": [אין תתי תיקיות קבועים – בדרך כלל subCategory = null]
- "תעודות לימודים": [אין תתי תיקיות קבועים – בדרך כלל subCategory = null]
...
- "מועדונים": ["לפי חברות"]
- "אישי": [אין תתי תיקיות – בדרך כלל subCategory = null]
- "כרטיסים": [אין תתי תיקיות – בדרך כלל subCategory = null]
- "נכסים": ["חוזים","משכנתא","שכירות","ביטוח"]
- "אחר": [אין תתי תיקיות – subCategory = null]

⚠️ כללים חשובים:
1. "category" חייב להיות *בדיוק* אחד מהשמות ברשימה למעלה.
2. אם אתה מחזיר "subCategory", הוא חייב להיות אחד השמות שמתאימים לקטגוריה שבחרת.
3. אם אין תת-תיקייה מתאימה או שהקטגוריה היא כזו בלי תתי-תיקיות (למשל "תעודות אחריות","תעודות לימודים","אישי","כרטיסים","אחר"):
   → subCategory = null.
4. אסור להמציא category או subCategory חדשים.  
   אם אין התאמה טובה → category = "אחר", subCategory = null.

דוגמאות כיוון:
- מסמך עם "תעודת אחריות", "warranty", "הוכחת קניה", "חשבון קניה":
  → בד"כ category = "תעודות אחריות".
- פוליסת ביטוח בריאות:
  → category = "ביטוחים", subCategory = "בריאות".
- תדפיס בנק / כרטיס אשראי:
  → category = "כלכלה", subCategory = "בנק" או "אשראי".
- בדיקות דם / סיכום ביקור רופא:
  → category = "רפואה", subCategory לפי התוכן.
- חוזה שכירות דירה:
  → category = "בית" או "נכסים", subCategory = "שכירות" (לפי ההקשר הכללי במסמכים).
...
- free:
  * החזר רק: category.
  * subCategory = null.
  * כל שאר השדות (organization, year, belongsTo, purchaseDate, warrantyUntil) = null.

- standard:
  * החזר category.
  * נסה לתת subCategory אם יש התאמה ברורה.
  * כל שאר השדות = null.

- advanced / pro / premium:
  * החזר category + subCategory (אם יש).
  * בנוסף נסה למלא:
    - organization: שם חברה / מוסד / ביטוח / בנק וכו'
    - year: שנת המסמך (למשל 2025)
    - belongsTo: למי המסמך שייך (שם אדם/עסק)
    - purchaseDate: תאריך קניה/התחלה בפורמט "YYYY-MM-DD" או null
    - warrantyUntil: תוקף אחריות עד בפורמט "YYYY-MM-DD" או null

📅 תאריכים:
- בישראל מופיעים לרוב כ: DD/MM/YYYY, DD.MM.YYYY, DD-MM-YYYY.
- אתה צריך להמיר תמיד ל-"YYYY-MM-DD" אם הצלחת לזהות, אחרת null.

🔙 הפלט חייב להיות *אך ורק* JSON חוקי במבנה:
