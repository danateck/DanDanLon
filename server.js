...
app.post('/api/ai/classify-document', async (req, res) => {
  try {
    ...
    const prompt = `אתה מודל שמסווג מסמכים למערכת ניהול מסמכים.  
אתה חייב לבחור category ו־subCategory רק מתוך הרשימה למטה, בלי להמציא שמות אחרים.
...
1. category חייב להיות אחת מהרשימה למעלה בלבד.
2. subCategory חייב להיות אחת מאלו שמתחת לאותה קטגוריה.
3. אסור להמציא קטגוריות או תתי-קטגוריות.
4. אם אין התאמה טובה — subCategory = null, אבל category חייב להיות מהרשימה.

5. כל מסמך רפואי (בדיקות, סיכומים, רופא, בית חולים):
   → category = "רפואה"

6. מסמכי ביטוח בריאות (פוליסה, ביטוח רפואי):
   → category = "ביטוחים", subCategory = "בריאות"

7. מסמכי רכישת מוצר + תעודת אחריות:
   → category = "אחריות"

8. אם תוכן המסמך מתאים למספר אפשרויות — בחר את ההתאמה בעלת הסבירות הגבוהה ביותר.
...
free:
  - החזר רק category.  
  - subCategory = null.  
  - אם לא בטוח → category = "אחר".

standard:
  - החזר category ו־subCategory (אם יש התאמה ברורה).

advanced / pro / premium:
  - החזר category + subCategory.
  - נסה למלא גם:
      organization (שם חברה/ארגון)
      year (שנת המסמך, 4 ספרות)
      belongsTo (למי המסמך שייך)
      purchaseDate (YYYY-MM-DD)
      warrantyUntil (YYYY-MM-DD)

premium:
  - דרוש דיוק מקסימלי.

===========================
📌 הפלט (חייב להיות JSON נקי)
===========================
...
שם קובץ: ${title || ""}
תוכן לדגימה: ${textSample || "(אין תוכן נוסף)"}
